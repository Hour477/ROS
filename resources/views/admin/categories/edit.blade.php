...
@section('content')
<div class="category-edit-page p-1 p-md-3">
    <!-- Header -->
    <div class="d-flex flex-wrap justify-content-between align-items-center gap-3 mb-4">
        <div class="flex-grow-1">
            <h2 class="fw-bold mb-0 responsive-h2" style="color: #1e293b;">{{ __('Edit Category') }}: {{ $category->name }}</h2>
            <p class="text-muted small mb-0">{{ __('Updating structural metadata for organizational unit') }} #{{ $category->id }}</p>
        </div>
                ...
                <div class="row g-4 g-lg-5">
                    ...
                    <div class="col-lg-8">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="name" class="form-control premium-field @error('name') is-invalid @enderror" value="{{ old('name', $category->name) }}" placeholder="{{ __('e.g. Italian Specialties') }}" required>
                        ...
                        <div class="mb-4 mb-md-5">
                            ...
                            <textarea name="description" class="form-control premium-field @error('description') is-invalid @enderror" rows="5" placeholder="{{ __('Briefly describe what items fall under this category...') }}">{{ old('description', $category->description) }}</textarea>
                        ...
                        <div class="d-flex flex-wrap gap-2 pt-3 border-top justify-content-end">
                            <button type="submit" class="btn btn-orange px-4 py-3 d-flex align-items-center gap-2 shadow-sm">
                                ...
                                <span class="fw-bold">{{ __('Update Category') }}</span>
                            </button>
                        ...
                        <div class="item-stats-list">
                            <div class="d-flex justify-content-between align-items-center py-3 border-bottom">
                                ...
                                <div class="form-check form-switch p-0 m-0">
                                    <input class="form-check-input premium-switch" type="checkbox" name="status" value="1" id="statusSwitch" {{ $category->status ? 'checked' : '' }}>
                                    <input type="hidden" name="status" id="statusHidden" value="0" {{ $category->status ? 'disabled' : '' }}>
                                </div>
                            </div>
                            <div class="py-2">
                                @if($category->status)
                                ...
                                @else
                                <span id="statusBadge" class="badge-status bg-secondary-subtle text-secondary justify-content-center w-100 mt-2">
                                    <span class="dot"></span> {{ __('Hidden / Disabled') }}
                                </span>
                                @endif
                            </div>
                        </div>
...
<script>
    document.getElementById('statusSwitch').onchange = function() {
        ...
        if (this.checked) {
            ...
            badge.innerHTML = '<span class="dot"></span> {{ __('Active & Visible') }}';
        } else {
            badge.className = 'badge-status bg-secondary-subtle text-secondary justify-content-center w-100 mt-2';
            badge.innerHTML = '<span class="dot"></span> {{ __('Hidden / Disabled') }}';
        }
